...
class LmSeqsDataset(Dataset):
    ...
    def preprepare_data(self, data):
        ...
        for doc in data:
            if doc[0] != cls_id:
                doc = np.concatenate([np.uint16([cls_id]), doc])
            if doc[-1] != sep_id:
                doc = np.concatenate([doc, np.uint16([sep_id])])
            new_data.append(doc)
        self.token_ids = np.array(new_data)
        self.lengths = np.array([len(t) for t in new_data])
    ...
    def remove_long_sequences(self):
        ...
        for seq_, len_ in zip(self.token_ids, self.lengths):
            assert (seq_[0] == cls_id) and (seq_[-1] == sep_id), seq_ # fixme
            if len_ <= max_len:
